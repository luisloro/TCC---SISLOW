...
function EditaEquipamentoExibicao() {
    ...
    const editaEquipamento = (dados: any) => axios.post(
        `${BASE_URL}/editequipexib?modelo=${modelo}&marca=${marca}&tipo=${tipo}&idEquipamento=${id}`).then(response => {
            console.log(response.data);
            setModelo('');
            setMarca('');
            setTipo('');
            navigate('/listaequipexib');
        });

        useEffect(() => {

            axios.get(
                `${BASE_URL}/equipexib/${id}`).then(response => {
...
export default EditaEquipamentoExibicao;
